...
|-------|--------------------------|------------------------------|-----------|
| `id` | `string` (CUID) | `Long` | **INCOMPATIBLE** (Needs Android `String`) |
...
| Field | RunFlow Web | RunFlow Android App | Alignment |
|-------|-------------|---------------------|-----------|
| `id` | `string` (UUID/CUID) | `String` (UUID) | Compatible |
| `goalId` | `string` | `String` | Compatible |
...
| 2 | **ID Type Mismatch** | **High** | Web uses `String` (CUID), Android uses `Long` | **Migrate Android IDs to String** |
...
1. **Verify ID Type Consistency**
   - **CRITICAL**: Android app MUST change `id` fields from `Long` to `String` for User and Activity models.
   - Web uses alphanumeric CUIDs (e.g., `clq...`) which cannot be parsed as Longs.
...
| **Data Models** | **Caution** | **ID Mismatch**: Web uses String CUIDs, Android uses Longs. |
