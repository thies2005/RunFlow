...
export async function syncUserActivities(userId: string, range?: string): Promise<{
    ...
    try {
        ...
        try {
            ...
            logger.error(`Sync failed: Token refresh error for user ${userId}`, tokenErr);
            throw new Error(`Authentication failed: ${tokenErr.message}`);
        }

        if (!accessToken) {
            logger.error(`Sync failed: No access token available for user ${userId}`);
