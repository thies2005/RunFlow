...
object WebPlanEngine {
    ...
    fun configFromSpec(spec: PlanSpec): WebPlanConfig {
        ...
        return WebPlanConfig(
            ...
            ridesPerWeek = spec.ridesPerWeek ?: if (isTriathlon) 2 else 0,
            swimsPerWeek = spec.swimsPerWeek ?: if (isTriathlon) 2 else 0,
            strengthPerWeek = spec.strengthPerWeek,
            weeklyMileageGoal = spec.weeklyKm * 1000,
            startWeeklyMileage = spec.startWeeklyKm?.times(1000),
            taperWeeks = spec.taperWeeks,
            peakWeeks = spec.peakWeeks,
            buildWeeks = spec.buildWeeks,
            maxLongRunKm = spec.longRunKm,
            longRunDay = spec.longRunDay.toJsDay(),
            workoutDay = spec.workoutDay.toJsDay(),
            swimDay = spec.swimDay?.toJsDay(),
            restDays = spec.restDays.map { it.toJsDay() }.sorted(),
            weeksTotal = if (spec.raceType == RaceType.NONE) PlanMath.planWeeks(spec) else null,
            customDistanceM = spec.customDistanceKm?.times(1000),
            customSwimDistM = spec.customSwimKm?.times(1000),
            customBikeDistM = spec.customBikeKm?.times(1000),
            customRunDistM = spec.customRunKm?.times(1000),
            thresholdHeartRate = spec.thresholdHeartRate,
            hrMax = spec.maxHeartRate,
            hrRest = spec.restingHeartRate,
        )
    }
