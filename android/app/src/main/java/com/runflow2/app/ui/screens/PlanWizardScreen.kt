                        ...
                        if (step < stepCount - 1) {
                            ...
                            scope.launch {
                                ...
                                if (created.isFailure && method != PlanMethod.CLASSIC) {
                                    ...
                                    // with an on-device plan generated by the local port
                                    // of the web engine, so offline plans stay identical
                                    // to what the server would have produced.
                                    AppLog.w(
                                        "Wizard",
                                        "web engine unavailable (${created.exceptionOrNull()?.message}) — created on-device web-engine plan instead",
                                    )
                                    container.repository.createPlanOffline(spec)
                                    Toast.makeText(
                                        container.appContext,
                                        "Couldn't reach the server — generated the plan on this phone with the same engine as the website.",
                                        Toast.LENGTH_LONG,
                                    ).show()
                                }
                                creating = false
