        ...
        <div className="space-y-3">
            ...
            <div>
                <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-medium text-zinc-300">Main Set</span>
                    <button
                        type="button"
                ...
                <div className="space-y-1.5">
                    {steps.main.map((step, i) => (
                        <div key={i} className="flex items-center gap-1.5 bg-zinc-800/50 rounded-md px-2 py-1.5">
                            <input
                                type="number"
                                min={1}
                                value={step.reps}
                                onChange={(e) => updateMainSet(i, 'reps', Math.max(1, Number(e.target.value)))}
                                className="w-12 bg-zinc-800 border border-zinc-700 rounded px-1.5 py-0.5 text-xs text-zinc-100 focus:outline-none focus:ring-1 focus:ring-zinc-500"
                                title="Reps"
                            />
                            ...
                            <input
                                ...
                                className="w-16 bg-zinc-800 border border-zinc-700 rounded px-1.5 py-0.5 text-xs text-zinc-100 focus:outline-none focus:ring-1 focus:ring-zinc-500"
                                title="Distance (m)"
                            />
                            ...
                            <select
                                ...
                                className="bg-zinc-800 border border-zinc-700 rounded px-1 py-0.5 text-xs text-zinc-100 focus:outline-none focus:ring-1 focus:ring-zinc-500"
                            >
                                {PACE_ZONES.map((z) => (
                            ...
                            <input
                                ...
                                className="w-14 bg-zinc-800 border border-zinc-700 rounded px-1.5 py-0.5 text-xs text-zinc-100 focus:outline-none focus:ring-1 focus:ring-zinc-500"
                                title="Rest (seconds)"
                            />
