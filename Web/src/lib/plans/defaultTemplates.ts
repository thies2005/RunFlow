/**
 * Default workout templates for the shared library (audit G8).
 *
 * Shared between:
 *   - prisma/seed-templates.ts (CLI seed script)
 *   - /api/admin/workout-templates/seed (runtime admin endpoint)
 *
 * Idempotent: upserts each template keyed on `name`.
 */

import type { PrismaClient, WorkoutType } from '@/generated/prisma/client';
...
// targetPace values are seconds-per-km. targetDistance is meters, targetDuration is seconds.
// NOTE: WorkoutType enum values are referenced as plain strings here so this module has no
// runtime dependency on the generated client (type-only import above). The values must match
// the Prisma `WorkoutType` enum exactly.
...
/**
 * Upsert all default templates. Idempotent (keyed on name).
 * @returns counts of created vs updated (best-effort heuristic).
 */
export async function seedDefaultTemplates(prisma: PrismaClient): Promise<{ created: number; updated: number; total: number }> {
    let created = 0;
    let updated = 0;
    ...
    return { created, updated, total: DEFAULT_TEMPLATES.length };
}
