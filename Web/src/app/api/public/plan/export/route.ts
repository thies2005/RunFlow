...
function generateHtml(plan: z.infer<typeof exportRequestSchema>['plan']): string {
    ...
    const weeksHtml = plan.weeks.map(week => {
        ...
        const workoutsHtml = week.workouts.map(w => {
            ...
            return `<tr>
                ...
                <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;font-size:14px;">${w.dayOfWeek}</td>
                <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;">
                    <span style="background:${typeColor};color:white;padding:2px 8px;border-radius:4px;font-size:12px;font-weight:600;">${w.type.replace(/_/g, ' ')}</span>
                </td>
                <td style="padding:8px 12px;border-bottom:1px solid #e5e7eb;font-size:14px;">${w.description}</td>
        ...
        return `<div style="margin-bottom:24px;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                ...
                <span style="background:${phaseColor}20;color:${phaseColor};padding:4px 12px;border-radius:6px;font-size:13px;font-weight:600;">${week.phase.replace(/_/g, ' ')}</span>
    ...
    const raceTypeTitle = plan.raceType.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
...
<body>
    ...
    <p class="subtitle">Race Date: ${plan.raceDate} | Generated by RunFlow</p>
    ${weeksHtml}
