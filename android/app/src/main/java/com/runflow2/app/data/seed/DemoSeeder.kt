...
class DemoSeeder(
    ...
    suspend fun seed(
        ...
        insertGoal(
            GoalEntity(
                ...
                planWeeks = PlanMath.planWeeks(spec),
        ...
        // Demo plan generated by the on-device web engine — the only engine
        // the app ships. webWorkoutType/webPhase keep the raw web enums so the
        // demo rows look exactly like a locally created web-engine plan.
        ...
        val workouts = generated.mapIndexed { i, w ->
            WorkoutEntity(
                ...
                isCompleted = w.date.isBefore(today) && w.type != "REST",
                completedAt = if (w.date.isBefore(today)) Format.epochMillis(w.date, LocalTime.of(9, 0)) else null,
                activityId = null,
                sortIndex = i,
            )
        }
    ...
    private suspend fun seedRun(
        ...
        val elevationM = distanceKm * (6 + rnd.nextDouble() * 10)
        val cadence = (168 + rnd.nextInt(-4, 6)).toDouble()
        ...
        insert(
            ActivityEntity(
                ...
                streamsJson = synthStreams(movingTime, paceSecFinal, avgHr, base.maxHr, cadence, elevationM, rnd),
            ),
        )
    }
    ...
    /**
     * Realistic per-run streams for the demo history so the analysis charts
     * (pace, GAP, elevation, HR, cadence) are populated on first launch:
     * warm-up + fatigue pace drift, rolling hills scaled to the elevation
     * gain, an HR ramp under the recorded max, cadence wobble.
     */
    private fun synthStreams(
        movingTimeSec: Int,
        paceSecPerKm: Double,
        avgHr: Double,
        maxHr: Int,
        ...
        if (movingTimeSec < 180) return null
        val interval = 10 // s per sample
        val n = movingTimeSec / interval + 1
        val cycles = (elevationGainM / 30.0).toInt().coerceIn(1, 12)
        val hillAmp = elevationGainM / cycles
        val time = DoubleArray(n)
        val velocity = DoubleArray(n)
        val altitude = DoubleArray(n)
        val hr = DoubleArray(n)
        val cad = DoubleArray(n)
        for (i in 0 until n) {
            val frac = i.toDouble() / (n - 1)
            val warm = if (frac < 0.08) 1.0 + 0.10 * (1 - frac / 0.08) else 1.0
            val fatigue = if (frac > 0.85) 1.0 + 0.05 * ((frac - 0.85) / 0.15) else 1.0
            val wobble = 1.0 + 0.03 * sin(frac * 2 * PI * 3) + (rnd.nextDouble() - 0.5) * 0.02
            velocity[i] = 1000.0 / (paceSecPerKm * warm * fatigue * wobble)
            altitude[i] = 48.0 + hillAmp * (0.5 - 0.5 * cos(2 * PI * cycles * frac)) + rnd.nextDouble()
            hr[i] = (avgHr - 10.0 + 16.0 * frac + (rnd.nextDouble() - 0.5) * 6).coerceAtMost(maxHr - 1.0)
            cad[i] = cadence + (rnd.nextDouble() - 0.5) * 5
        }
        return ActivityStreamsDto(
            time = time.toList(),
            altitude = altitude.toList(),
            heartrate = hr.toList(),
            cadence = cad.toList(),
            velocitySmooth = velocity.toList(),
        ).toJsonOrNull()
    }
