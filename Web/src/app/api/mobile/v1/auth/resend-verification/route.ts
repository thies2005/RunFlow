...
export async function POST(request: NextRequest) {
    try {
        ...
        const rateLimitResult = await checkRateLimitAsync(clientId, {
            limit: 10,
            windowSeconds: 3600,
            prefix: 'mobile-resend-verification',
        });
        ...
        try {
            ...
            logger.error('[Mobile ResendVerification] Failed to send verification email', { error });
        }

        return NextResponse.json({ success: true });
    } catch (error) {
        logger.error('[Mobile ResendVerification] Unexpected error', { error });
