...
export function SupplementsSection({
    ...
    const renderStack = (stack: StackWithSupplements) => {
        ...
        return (
            <div key={stack.id} className="mb-4 bg-foreground/5 border border-foreground/10 rounded-xl overflow-hidden group">
                <div className="flex items-center justify-between p-3 bg-foreground/5 border-b border-foreground/10 hover:bg-foreground/10 transition-colors">
                    <button type="button" className="flex-1 text-left" onClick={() => onEditStack(stack)}>
                        <h4 className="text-sm font-bold text-foreground group-hover:text-blue-400 transition-colors flex items-center gap-1.5">
                            {stack.name} {stack.timeOfDay && <span className="text-[10px] uppercase font-bold text-foreground-muted bg-foreground/10 px-1.5 py-0.5 rounded ml-1">{stack.timeOfDay}</span>}
                        </h4>
                        <p className="text-[10px] text-foreground-muted mt-0.5">{stack.supplements?.length || 0} items</p>
                    </button>
                    ...
                    {hasSupplements && (
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors border ${allTaken ? 'bg-blue-500 border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)] text-white' : 'bg-transparent border-foreground/30 text-foreground-muted hover:border-foreground/40'} disabled:opacity-50`}
                            >
                                {allTaken ? <HeartPulse className="w-4 h-4" /> : <div className="w-2.5 h-2.5 rounded-full bg-background-secondary0" />}
                            </button>
                        </div>
                    )}
                </div>
        ...
        <div className="glass-card border border-glass-border rounded-2xl p-4 flex flex-col">
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                    ...
                    <button type="button" onClick={onAddStack} className="bg-foreground/5 hover:bg-foreground/10 text-foreground flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border border-foreground/10 transition-colors">
                        <Plus className="w-3.5 h-3.5" /> Stack
                    </button>
                    <button type="button" onClick={onAddSupplement} className="bg-foreground/10 hover:bg-foreground/15 text-foreground flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors">
            ...
            {isLoading ? (
                ...
                <div className="flex gap-3 mt-2">
                    <button type="button" onClick={onAddSupplement} className="flex-1 text-center py-6 border border-dashed border-foreground/10 hover:border-foreground/20 hover:bg-foreground/5 rounded-lg transition-colors group flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="text-sm text-foreground-muted tracking-wide font-medium group-hover:text-foreground transition-colors">Add Supplement</p>
                    </button>
                    <button type="button" onClick={onAddStack} className="flex-1 text-center py-6 border border-dashed border-foreground/10 hover:border-foreground/20 hover:bg-foreground/5 rounded-lg transition-colors group flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="text-sm text-foreground-muted tracking-wide font-medium group-hover:text-blue-400 transition-colors">Create Stack</p>
                    </button>
                </div>
            ) : (
                <div className="space-y-4">
                    ...
                    {morningStandalone.length > 0 && <div><h4 className="text-[10px] font-bold text-foreground-muted uppercase tracking-widest mb-2 px-1">Morning Standalones</h4>{morningStandalone.map(renderSupplementItem)}</div>}
                    {noonStandalone.length > 0 && <div><h4 className="text-[10px] font-bold text-foreground-muted uppercase tracking-widest mb-2 mt-4 px-1">Noon Standalones</h4>{noonStandalone.map(renderSupplementItem)}</div>}
                    {eveningStandalone.length > 0 && <div><h4 className="text-[10px] font-bold text-foreground-muted uppercase tracking-widest mb-2 mt-4 px-1">Evening Standalones</h4>{eveningStandalone.map(renderSupplementItem)}</div>}
                </div>
            )}
        </div>
    );
